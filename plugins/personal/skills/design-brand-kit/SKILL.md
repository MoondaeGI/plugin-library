...
description: 제품 설명을 바탕으로 브랜드 정체성·톤·색상·타이포그래피·로고 방향·UI 분위기·금지 패턴을 정리한 브랜드 킷을 만들고, 로고(필수)·브랜드 킷 오버뷰 보드 같은 브랜드 이미지를 한 개씩 협업하며 생성·반복할 때 사용한다.
---
...
제품 설명만 보고 바로 화면을 만들지 않는다. 먼저 브랜드의 성격·시각 방향·색상·타이포그래피·로고 방향·UI 분위기를 정리한 뒤, 로고·브랜드 킷 오버뷰 보드 같은 브랜드 이미지를 **실제 디자이너처럼 한 개씩 만들어 보여주고, 피드백을 받아 반복 수정**한다. 이미지 아트 디렉션은 `references/brand-kit-image.md`를 따른다.
...
- `.design/image-briefs/brand-briefs.md` — 로고·오버뷰 보드·추가 탐색 브리프
- `.design/generated/logo/` — 단색 클린 로고 이미지 (필수)
- `.design/generated/brand-kit/` — 브랜드 킷 오버뷰 보드·추가 탐색 이미지
...
## 브랜드 킷 오버뷰 보드 (필수)
### 레이아웃 (기본 3×3 / 2×3 등)
### 비주얼 모드 (references/brand-kit-image.md의 8개 모드 중 선택)
### 패널 구성 메모 (로고 커버·구성·디지털 적용·태그라인·컬러·타이포·물리 적용·이미지 디렉션·시스템 디테일)
### 태그라인 (짧고 구체적으로)
### 이미지 생성 Prompt
### Negative Prompt
(텍스트는 크고 희소하게 — 정확한 색/폰트 스펙은 BRAND_KIT.md/tokens에)

## 추가 탐색 이미지 (선택)
### 용도 (대안 무드 / 키 비주얼 / 이미지 디렉션 / 히어로 배경 등)
### 형태·패턴·텍스처 방향 (로고·보드와 일관)
### 이미지 생성 Prompt
### Negative Prompt
```
...
- `Use case` 슬러그 (Codex `image_gen`의 use case 값): 로고 = `logo-brand`, 오버뷰 보드·추가 탐색 = `stylized-concept`.
- **저장**: 생성 기본 위치에 방치하지 말고 워크스페이스로 복사 — 로고는 `.design/generated/logo/`, 오버뷰 보드·추가 탐색은 `.design/generated/brand-kit/`. 파일명은 항목 식별 가능하게(`logo-concept-1.png`, `brand-board-1.png`), 재생성 시 버전(`-v2`)으로 기존 확정본을 덮지 않는다.
- 보드의 상세 구성·비주얼 모드·프롬프트 템플릿은 `references/brand-kit-image.md` 참조.
...
2. `.design/image-briefs/brand-briefs.md` 작성 (로고·오버뷰 보드·추가 탐색 브리프).
3. **항목을 한 개씩** 진행한다. 순서: **로고(필수) → 브랜드 킷 오버뷰 보드(필수) → (선택) 추가 탐색 이미지**. 각 항목마다:
