...
- **autocrop**: `--autocrop`(+ `--autocrop-pad-pct N`, 기본 6). 저장된 PNG의 **투명/단색 여백을 잘라 마크가 캔버스를 꽉 채우게** 한다 — gpt-image는 마크를 작게 그려 넓은 여백을 남기므로, 컷아웃 자산(로고·워드마크·아이콘)은 이게 없으면 HTML에서 콩알만 해진다. 사진/UI 목업엔 쓰지 않는다. (별도 CLI: `scripts/autocrop.mjs`.)
