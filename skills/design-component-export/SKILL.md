...
description: （placeholder · 미구현 — 아직 호출하지 말 것）확정된 ui-kit.css·tokens.css를 대상 프로젝트의 plain CSS 컴포넌트 세트 + 얇은 React/Next 래퍼로 export하는 파이프라인 최종 단계. 범위·게이트·산출물은 아직 설계 전이며 별도 논의에서 확정한다.
---
...
디자인 파이프라인의 최종 목표인 **대상 프로젝트에 실제 컴포넌트 세트 생성**을 담당할 단계의 자리표시자.
