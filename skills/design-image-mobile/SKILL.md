...
- **`.design/DESIGN.md`** 가 단일 시드다. 이 파일에서 frontmatter 토큰(colors HEX·typography 실폰트)과 §1 제품명·§3 시각 방향·§8 이미지 에셋 규칙·§11 anti-slop을 읽어 프롬프트를 구성한다. 값을 지어내지 않는다.
...
- `.design/DESIGN.md` — frontmatter 토큰(colors HEX·typography 실폰트) / §1 제품명 / §3 시각 방향 / §8 이미지 에셋 규칙 / §11 anti-slop
