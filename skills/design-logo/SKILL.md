---
name: design-logo
description: 확정된 brand kit를 바탕으로 로고를 탐색·확정하는 스킬. brand-kit이 만든 assets/logo-base.png(투명)를 시드로, 한 라운드에 3~4개의 큰 방향(메타포까지 발산)을 개별 투명 PNG로 만들어 저작한 logos.html 탐색 시트(번호·라벨·실색·실폰트)로 보여주고, #N을 골라 수렴 라운드 또는 단독 확정으로 좁혀 확정 단일 로고를 .design/final/logo/assets/에 확정할 때 사용한다.
---
...
- `.design/final/brand-kit/assets/logo-base.png` — **확정 로고 마크 시드(투명).** 모드 A·베이스라인 타일 시드.
- `.design/final/brand-kit/BRAND_KIT.md` — §6 로고 방향(구성·의미·금지), §1 개요, 금지 패턴.
...
10. **확정(복사)**: 확정본을 `.design/final/logo/assets/logo.png`로 복사. 시안은 `.design/logo/assets/`에 보존. `logo-briefs.md`에 확정 컨셉을 기록한다.
