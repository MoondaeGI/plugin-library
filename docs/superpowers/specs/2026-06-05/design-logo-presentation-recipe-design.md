...
- favicon/단색마크 전략 (유력안 — SVG 회피):
  ...
  - 경계(가정): mask/filter 단색 재색은 **단색에만** 성립. 풍부한 다색 엠블럼 다크는 그걸론 안 됨. **[2026-06-07 갱신]** "그래서 디자인된 다크 2장뿐"이라던 가정은 **반증됨** — 결정론 **팔레트 리맵**(영역별 소스색→다크 hex, OKLab·엣지 보간)으로 다색 다크가 됨이 선검증됨(스펙 B-🅱-i). 생성 2장은 구조 재설계용 폴백으로만.
  - 여전히 선검증: "풍부한 엠블럼 → 작은 단색 마크로 모티프 축약" 가능성(트레이싱은 ~192px 열화로 탈락 — 손/생성 어느 경로로 축약할지).
- 다크 변형(풍부한 로고 다크) — **[2026-06-07] 스펙 B-🅱-i로 분리**, 결정론 리맵 기본 + 생성 폴백.
