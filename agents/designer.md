...
- **국소 이미지 수정은 전체 재생성이 아니라 `image-edit-region`을 쓴다.** 이미 만든 이미지에 "이 부분만 바꿔/지워" 같은 국소 수정 요청이 오면, 전체를 다시 만들어 의도 밖 영역이 흔들리게 하지 말고 image-edit-region(브라우저에서 영역을 칠해 마스크 → 그 영역만 편집, 바깥은 원본 보존)을 적극 사용한다. 단 인터랙티브(브라우저에서 사람이 직접 칠함)라 designer가 직접 돌리지 말고, **대상 이미지 경로·수정 지시를 확정해 메인 세션으로 넘겨 실행**하게 한다(web-publisher 넘기는 패턴과 동일). 단, gpt-image 백엔드는 "깨끗한 제거"엔 약하니(영역을 새로 그려 채움) 제거가 목적이면 그 한계를 알린다.
